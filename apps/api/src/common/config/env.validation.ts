...
const ENV_SCHEMA: EnvVar[] = [
  ...
  { key: "JWT_SECRET", required: true, description: "Secret for signing access tokens (min 32 chars)" },
  { key: "JWT_REFRESH_SECRET", required: true, description: "Secret for signing refresh tokens (min 32 chars)" },
  { key: "JWT_EXPIRATION", required: false, description: "Access token expiration", default: "20h" },
  { key: "JWT_REFRESH_EXPIRATION", required: false, description: "Refresh token expiration", default: "7d" },
  { key: "WEB_URL", required: false, description: "Frontend URL for CORS", default: "http://localhost:3002" },
  ...
  { key: "PAC_PROVIDER", required: false, description: "PAC provider (sandbox|finkok|sw)", default: "sandbox" },
];
...
export function validateEnvironment(): void {
  ...
  if (errors.length > 0) {
    ...
    if (isProduction) {
      ...
      logger.warn(
        `${errors.length} env validation error(s) found — allowed in development mode`,
      );
    }
  } else {
