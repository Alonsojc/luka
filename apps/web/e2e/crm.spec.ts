...
test.describe("CRM y Lealtad", () => {
  ...
  test("CRM - tabs de Clientes, Lealtad, Promociones", async ({ page }) => {
    ...
    await expect(page.getByText("Clientes", { exact: true }).first()).toBeVisible({ timeout: 15000 });
    await expect(page.getByText("Lealtad", { exact: true }).first()).toBeVisible({ timeout: 15000 });
    await expect(page.getByText("Promociones", { exact: true }).first()).toBeVisible({ timeout: 15000 });
  });
  ...
  test("Lealtad - muestra miembros del programa", async ({ page }) => {
    ...
    await expect(page.getByText("Miembros", { exact: true }).first()).toBeVisible({ timeout: 15000 });

    // A table or data content should be visible
    const hasTable = await page.locator("table").first().isVisible().catch(() => false);
    const hasContent = await page.locator("text=/miembros|programa/i").first().isVisible().catch(() => false);
