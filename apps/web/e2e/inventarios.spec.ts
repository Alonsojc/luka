...
test.describe('Inventarios', () => {
  ...
  test('crear producto via modal aparece en la tabla', async ({ page }) => {
    ...
    const nameInput = modal.locator('label:has-text("Nombre") + input, label:has-text("Nombre") ~ input').first();
    ...
    // Submit the form
    const submitButton = modal.locator('button[type="submit"], button:has-text("Guardar"), button:has-text("Crear")').first();
  ...
  test('editar producto actualiza en la tabla', async ({ page }) => {
    ...
    const firstRowEditButton = page.locator('table tbody tr').first().locator('button').filter({
    ...
    // Submit the form
    const submitButton = modal.locator('button[type="submit"], button:has-text("Guardar"), button:has-text("Actualizar")').first();
    if (await submitButton.isVisible()) {
      await submitButton.click();
    }
  ...
  test('cambiar a tab Recetas muestra recetas', async ({ page }) => {
    ...
    const contentVisible = await page.locator('table').isVisible() ||
